...
# Function to generate BRD part with error handling and retries
def generate_brd_part(prompt, placeholder, max_retries=3, retry_delay=5):
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                st.warning(f"Connection error occurred. Retrying in {retry_delay} seconds... (Attempt {attempt + 1}/{max_retries})")
                st.error(f"Error details: {str(e)}")
                time.sleep(retry_delay)
            else:
                st.error(f"Failed to generate BRD part after {max_retries} attempts. Please try again later.")
                st.error(f"Final error details: {str(e)}")
                raise e
...
# Functions to convert Markdown to PDF and DOCX
def markdown_to_pdf(markdown_text, output_filename):
    ...
    paragraphs = [Paragraph(line, styles['Normal']) for line in html.split('\n') if line.strip()]
...
if st.button("Generate BRD", key="generate_brd"):
    if all(st.session_state.form_fields.values()):
        ...
        **Project Date:** {st.session_state.form_fields['project_date']}
        ...
        pdf_data = markdown_to_pdf(full_brd, f"BRD_{st.session_state.form_fields['client_name']}_{st.session_state.form_fields['project_date']}.pdf")
        docx_data = markdown_to_docx(full_brd, f"BRD_{st.session_state.form_fields['client_name']}_{st.session_state.form_fields['project_date']}.docx")
        ...
        with col2:
            st.download_button(
                ...
                data=pdf_data,
                file_name=f"BRD_{st.session_state.form_fields['client_name']}_{st.session_state.form_fields['project_date']}.pdf",
        ...
        with col3:
            st.download_button(
                ...
                data=docx_data,
                file_name=f"BRD_{st.session_state.form_fields['client_name']}_{st.session_state.form_fields['project_date']}.docx",
...
if st.checkbox("Debug: Check API Key"):
